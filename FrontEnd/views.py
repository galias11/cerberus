...
def crear_documento_general(request,form,caso_id):
    ...
    nuevo_doc.hash_md5 = hash_md5.hexdigest()
    nuevo_doc.hash_sha1 = hash_sha.hexdigest()
    nuevo_doc.propietario_doc = request.user
    nuevo_doc.eliminado = False

    #Parsea el documento en base a si es txt o docx
    nombre, punto, ext = nuevo_doc.nombre_doc.rpartition(".")
    ext = ext.lower()
    lector = lectores_dict[ext](file)
    nuevo_doc.texto = lector.read()
    nuevo_doc.save()
...
def crearEntidades(documento,caso_id):
    ...
    for token in resultado_tokenizer:
        tipo = tipo_token[token[1]] if token[1] in tipo_token.keys() else "MISC"
        parrafo = Parrafo.objects.get(id=token[5])
        token_aux = TokensDoc(doc=documento, aparicion=token[3], tipo=tipo, frase=token[2], lema=token[0], categoria=token[4], parrafo=parrafo, eliminado=False)
        token_aux.save()
...
def ver_doc(request, id_doc):
    ...
    if ext == "docx":
        response = HttpResponse(doc.documento, content_type='application/vnd.openxmlformats-officedocument.wordprocessingml.document')
    elif ext == "txt":
        response = HttpResponse(doc.documento, content_type='text/plain')
...
def buscador_inteligente(request,tipo,caso_id):
    ...
    if tipo == "Lexicos detectados":
        ...
        for documento in documentos:
            entidad = EntidadesDoc.objects.filter(doc=documento).filter(tipo=entidades_dic[tipo]).filter(eliminado=False)
            for e in entidad:
